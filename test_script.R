...
#cat("\n\nstart server...\n")
#system(command = "r/app.R &>tmp.log &")
#Sys.sleep(1)

cat("\ntry to sign in with unregistered user\n")
POST(url = "http://127.0.0.1:8000/signIn", body = list(password="password", name="Alice"), encode = "form", verbose())
cat("\n########################\n\n")
str(res)

# cat("\n\ntry to sign up with invalid too short password")
# POST(url = "http://127.0.0.1:8000/signUp?password=pw&name=Alice&country=Germany")
# 
